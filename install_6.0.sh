...
v1=$(curl -sS --connect-timeout 10 -m 3 http://www.bt.cn/api/index/get_time)
if [ "$v1" != "" ];then
...
pip install python-socketio -I
pip install flask-socketio
pip install Werkzeug==0.15.1
pip install Pillow==5.4.1
pip install requests -I --user
pip install cffi==1.12.3 -I
pip install cryptography
pip install gunicorn==19
for p_name in psutil chardet virtualenv Flask Flask-Session Flask-SocketIO flask-sqlalchemy Pillow gunicorn gevent-websocket cryptography pyopenssl;
do
	pip install ${p_name}
done
